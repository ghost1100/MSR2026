...
\thanks{This work analyzes preprocessing requirements for the MSR 2026 Challenge dataset. Research conducted at [REDACTED INSTITUTION], December 2025.}
}
...
We analyze the MSR 2026 AI Development dataset, which contains 932,791 pull request records with assigned AI agent labels across five categories: OpenAI_Codex, Copilot, Cursor, Devin, and Claude_Code. 
...
\textbf{Volume Distribution Analysis}: Compute per-user pull request counts and formal concentration metrics including Gini coefficient, Shannon entropy, and percentile shares to quantify contribution skew and distributional diversity.
...
\textbf{Stage 1 - Concentration Analysis}: Compute formal concentration metrics (Gini coefficient, Shannon entropy, top-1\% share) to quantify user contribution skew and distributional diversity in the raw dataset. This stage is diagnostic only and does not remove any accounts.

\textbf{Stage 2 - Statistical Outlier Filtering}: Remove contributors exceeding the 99th percentile threshold ($\geq$178 PRs per user) to assess impact of high-volume accounts. The 99th percentile threshold was selected as a conventional statistical outlier boundary. This choice represents one reasonable approach to identifying high-volume contributors, though other thresholds could be justified.
...
\begin{itemize}
    ...
    \item Gini coefficient: 0.829 indicating high concentration
    \item Shannon entropy: 0.769 (normalized: 0.331) indicating low diversity
...
\begin{itemize}
    \item OpenAI\_Codex: 814,522 PRs (87.3\%)
    \item Copilot: 50,447 PRs (5.4\%)  
    \item Cursor: 32,941 PRs (3.5\%)
    \item Devin: 29,744 PRs (3.2\%)
    \item Claude\_Code: 5,137 PRs (0.6\%)
\end{itemize}
...
\begin{itemize}
    \item Gini coefficient: 0.829 (high inequality)
    \item Shannon entropy: 0.769 (low diversity, 33.1\% of maximum)
    \item Top-1\% users (721 accounts): 43.6\% of total PRs
    \item Top-0.1\% users (72 accounts): 20.2\% of total PRs
    \item 99th percentile threshold: 178 PRs per user
\end{itemize}
...
\begin{itemize}
    \item Accounts removed: 725 (1.0\% of users)
    \item Pull requests removed: 407,074 (43.6\% of all PRs)
    \item Mean user contribution after filtering: 7.4 PRs
\end{itemize}
...
\midrule
OpenAI Codex & 87.3 & 93.5 & +6.2 \\
Copilot & 5.4 & 0.1 & -5.3 \\
Cursor & 3.5 & 5.5 & +2.0 \\
Devin & 3.2 & 0.0 & -3.2 \\
Claude Code & 0.6 & 0.8 & +0.3 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    \item Bot naming patterns: 166 accounts identified via regex patterns
    \item Pattern categories: bot usernames (163), [bot] suffix (12), CI/automation (4 accounts)
    \item Volume-based automation indicators: Accounts exceeding 178 PR threshold
\end{itemize}

\textbf{Final Debiased Distribution}: OpenAI\_Codex (93.5\%), Cursor (5.5\%), Claude\_Code (0.8\%), Copilot (0.1\%)
...
In this dataset, user contribution filtering produces distributional changes averaging 4.4 percentage points across the agent categories. Statistical outlier filtering (99th percentile) produces distributional changes ranging from 0.3 to 6.2 percentage points per agent. The most dramatic change is the near-elimination of high-volume Copilot and Devin contributions, suggesting these agents were associated with automated accounts. The practical significance of these changes depends on the specific research context and analysis goals.

\textbf{Key Findings}:
\begin{itemize}
    \item Raw distributions show high user contribution skew (Gini = 0.829) and low diversity (Shannon entropy = 0.769)
    \item Agent distribution exhibits low Shannon entropy (normalized = 0.331), confirming distributional imbalance
    \item 99th percentile filtering changes agent distributions by 4.4\% mean absolute deviation
    \item Pattern-based filtering identifies 166 accounts with automation-like characteristics (0.2\% of users)
...
\textbf{Methodological Implications}: User-level filtering appears necessary for this dataset to obtain more representative distributional characterizations. The combination of high inequality (Gini = 0.829) and low diversity (Shannon entropy = 0.331) demonstrates that raw repository data requires preprocessing consideration in empirical studies.
