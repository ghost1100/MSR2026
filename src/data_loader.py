"""
Data loading utilities for MSR project
Handles AIDev dataset loading from local files or HuggingFace
"""
import pandas as pd
import os
...
def load_aidev(local_path="data/raw/aidata.csv", from_huggingface=False, config="all_pull_request", sample_size=10000):
    ...
    """
    # Handle path resolution when called from notebooks subdirectory
    if not os.path.exists(local_path) and os.path.exists(f"../{local_path}"):
        local_path = f"../{local_path}"
    
    if from_huggingface:
        # Download dataset from HuggingFace (753MB, ~900K records)
        ...
        # Load from local CSV file (much faster)
        ...
        if sample_size:
            # Load full dataset first to preserve agent distribution in sample
            ...
            # Load entire dataset (may take time for 753MB file)
