...
\begin{abstract}
The MSR 2026 Challenge dataset contains \textbf{900k+} pull requests labeled with AI agent attribution from the official MSR 2026 HuggingFace PR-level release (aidata.csv). This paper characterizes this dataset through systematic analysis of all records in the dataset. We document real agent distribution: \textbf{OpenAI Codex 87.3\%} (814,522 PRs), \textbf{Copilot 5.4\%} (50,447 PRs), \textbf{Cursor 3.5\%} (32,941 PRs), \textbf{Devin 3.2\%} (29,744 PRs), and \textbf{Claude Code 0.6\%} (5,137 PRs). Analysis reveals \textbf{72,189 unique users} with diverse contribution patterns and high completeness of core text fields. This characterization enables informed empirical research design and methodological decisions for the MSR 2026 Challenge dataset.
\end{abstract}
...
Our analysis reveals important structural properties: PRs are distributed across \textbf{5 distinct AI agents} with \textbf{OpenAI Codex dominating at 87.3\%} (814,522 PRs), followed by \textbf{Copilot at 5.4\%} (50,447 PRs), \textbf{Cursor at 3.5\%} (32,941 PRs), \textbf{Devin at 3.2\%} (29,744 PRs), and \textbf{Claude Code at 0.6\%} (5,137 PRs). The dataset spans \textbf{72,189 unique users} with diverse contribution patterns across different AI development tools. 

A notable characteristic of this dataset is the broad distribution across multiple AI agents, with OpenAI Codex representing the largest single category at 87.3% of all pull requests, while the remaining agents show more balanced usage patterns among their respective user communities. This concentration likely reflects automation or institutional data collection rather than individual developer behavior, and must be considered when designing empirical studies. Understanding these distributional patterns is essential for appropriate statistical analysis and methodological design.
...
We analyzed the official MSR 2026 HuggingFace PR-level release (aidata.csv) containing \textbf{900k+ pull requests} (147.3 MB). The dataset includes fields for agent labels, user IDs, timestamps, PR titles, descriptions, closure status, and repository metadata across 14 columns.
...
The dataset shows a five-agent distribution:
\begin{itemize}
    \item \textbf{OpenAI Codex}: 814,522 PRs (87.3\%)
    \item \textbf{Copilot}: 50,447 PRs (5.4\%)
    \item \textbf{Cursor}: 32,941 PRs (3.5\%)
    \item \textbf{Devin}: 29,744 PRs (3.2\%)
    \item \textbf{Claude Code}: 5,137 PRs (0.6\%)
\end{itemize}

This multi-agent distribution reflects the comprehensive AI development landscape captured in the MSR 2026 dataset, with OpenAI Codex representing the dominant presence. While both labels are numerically represented, extreme user-level clustering severely limits any valid comparative behavioral analysis between agents.
...
\textbf{Metadata Distribution Analysis}: Computed descriptive statistics for title lengths, body lengths, and closure rates across agent categories.
...
\textbf{Multi-Agent Distribution}: The dataset contains five AI agents with significant imbalances: OpenAI Codex dominates with 814,522 PRs (87.3\%), followed by Copilot with 50,447 PRs (5.4\%), Cursor with 32,941 PRs (3.5\%), Devin with 29,744 PRs (3.2\%), and Claude Code with 5,137 PRs (0.6\%). This distribution enables comparative analysis across multiple AI development tools.
...
\textbf{User Distribution Analysis}: The complete dataset spans \textbf{72,189 unique users} across five AI development platforms:
\begin{itemize}
    \item \textbf{Average contributions}: 12.9 PRs per user with platform-specific patterns
...
\begin{itemize}
    \item Automated or institutionally generated PR activity associated with the Copilot label
...
\midrule
Core Fields & Minimal & 932,791 (>99\%) \\
User & 0 (0\%) & 932,791 (100\%) \\
Agent & 0 (0\%) & 932,791 (100\%) \\
State & 0 (0\%) & 932,791 (100\%) \\
Metadata & Variable & High coverage \\
\midrule
\textbf{Overall} & \textbf{High quality} & \textbf{>99\%} \\
\bottomrule
\end{tabular>
\end{table}
...
\textbf{Sample Size Adequacy}: With 932,791 total records distributed across five AI agent categories (814,522 OpenAI Codex, 50,447 Copilot, 32,941 Cursor, 29,744 Devin, 5,137 Claude Code), the dataset provides substantial statistical power for comparative analysis across multiple AI development platforms.
...
\textbf{Label Representation}: The 7:1 ratio between Copilot and Claude Code provides numerical representation of both categories, though extreme user-level clustering prevents valid comparative behavioral analysis.
...
\textbf{Agent Label Distribution}: The 87.8\% Copilot vs. 12.2\% Claude Code distribution provides numerical representation of both labels, but does not support valid behavioral comparison due to extreme user-level clustering. The 7:1 ratio avoids extreme imbalances, but clustering effects prevent meaningful comparative inference.
...
\textbf{Sample Size Considerations}: The 900k+ total records provide large numerical coverage for descriptive and structural analyses while remaining computationally manageable. The binary label distribution simplifies structural analysis while avoiding multi-class complexity.
...
\textbf{Label Distribution}: The 7:1 Copilot to Claude Code ratio provides numerical representation of both categories, though extreme user-level clustering prevents valid comparative behavioral analysis between agents.
...
\begin{enumerate}
    ...
    \item \textbf{Leverage Binary Label Structure}: The two-agent structure simplifies structural analysis while providing sufficient contrast
...
This comprehensive characterization of the official MSR 2026 HuggingFace PR-level release provides essential insights for empirical research design. Analysis of all 900k+ pull requests reveals manageable distributional properties and adequate data quality for robust empirical studies.

\textbf{Key Findings}: The dataset provides adequate field-level completeness for exploratory structural analysis, though extreme user concentration (86.7\% from one account) prevents behavioral generalizations about tool effectiveness.

\textbf{Methodological Implications}: The dataset may support exploratory, methodologically constrained analyses after explicitly accounting for extreme user-level clustering. The binary label structure enables limited structural analysis, though the extreme user concentration prevents generalizable behavioral conclusions about tool effectiveness.

\textbf{Research Contribution}: This analysis provides empirically computed dataset characteristics, enabling informed research design and preventing methodological errors based on incorrect assumptions about dataset properties.

\textbf{Future Directions}: The documented characteristics support development of methodologically constrained analyses examining workflow-level and process-level differences under strict controls for user clustering and automation.
