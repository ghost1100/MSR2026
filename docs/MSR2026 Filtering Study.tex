...
\begin{abstract}
Large-scale repository datasets used to study AI tool adoption often contain extreme user contribution imbalances that distort apparent usage patterns. This paper investigates how automated accounts and high-volume system contributors create misleading signals in AI agent distribution analysis. Using the MSR 2026 AI Development dataset, we demonstrate that naive analysis of agent labels yields distributional characterizations that are highly sensitive to user-level contribution patterns. We apply a progressive filtering approach that removes non-human-scale contributors and compare agent distributions before and after debiasing. Our results show that the raw dataset exhibits significant user contribution skew that affects agent label distributions. After systematic user-level filtering, observed AI agent distributions change measurably, indicating that raw distributions reflect user activity patterns as well as tool usage patterns. This study establishes user-level filtering as a necessary preprocessing consideration for distributional analysis of AI agent labels in repository datasets.
\end{abstract}
...
The MSR 2026 AI Development dataset, containing over 900,000 pull requests with AI agent labels, exemplifies this challenge. Initial analysis reveals significant user contribution imbalances that affect apparent agent label distributions. Varying levels of user activity motivate the need for systematic filtering approaches to obtain cleaner distributional characterizations.

This paper addresses a critical methodological gap in AI tool adoption studies: the failure to account for user-level contribution patterns when analyzing agent distributions. We demonstrate that naive analysis of raw agent labels produces distributional characterizations that are systematically influenced by user contribution patterns.
...
We analyze the MSR 2026 AI Development dataset, which contains 932,791 pull request records with AI agent labels across five categories: OpenAI Codex, GitHub Copilot, Cursor, Devin, and Claude Code.

The dataset spans 72,189 unique user identifiers, with agent labels assigned to each pull request. Our analysis focuses on how user contribution patterns affect the apparent distribution of these agent labels.
...
\textbf{Account Pattern Classification}: Identify accounts matching systematic patterns: usernames containing 'bot' (case-insensitive), '[bot]' suffix notation, or ending with organizational indicators ('-ci', '-automation', company names). Manual validation on a random sample of flagged accounts yielded 91\% precision for the detection heuristic.
...
\textbf{Stage 2 - Statistical Outlier Filtering}: Remove contributors exceeding the 99th percentile threshold (≥215 PRs per user) to assess impact of high-volume accounts. The 99th percentile threshold was selected based on the empirical distribution, which exhibited a sharp inflection point separating typical users from high-volume outliers in the cumulative contribution curve.
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Agent} & \textbf{Raw \%} & \textbf{Filtered \%} & \textbf{Δ\%} \\
\midrule
...
User contribution imbalances affect agent label distributions by an average of 2.5 percentage points across the five agent categories. Statistical outlier filtering (99th percentile) produces distributional changes ranging from 0.4 to 5.2 percentage points per agent.
...
\textbf{Filtering Criteria}: Our thresholds for identifying automation may inadvertently remove legitimate high-productivity human developers, potentially introducing different forms of bias.

\textbf{Agent Label Accuracy}: The filtering process assumes that agent labels in the dataset accurately reflect tool usage, though label validity remains independently unverified.
...
This study demonstrates that user-level contribution imbalances affect AI agent distribution analysis in repository datasets. The MSR 2026 dataset exhibits significant user contribution skew that influences apparent agent distributions when user contribution patterns are not systematically addressed.
...
The MSR 2026 Challenge dataset is publicly available from HuggingFace. Our filtering analysis was performed using Python with pandas for data processing. The complete filtering script and debiased dataset statistics are available at: \url{https://github.com/ghost1100/MSR/}
