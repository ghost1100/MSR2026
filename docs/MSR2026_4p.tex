...
\textbf{Limitations for Practice}: Our observational data cannot support specific organizational recommendations. Testing presence does not guarantee testing quality. Observed patterns may reflect user selection rather than agent capabilities.

\textbf{Evaluation Considerations}: Organizations evaluating AI tools should consider testing behavior patterns as one signal among many (cost, integration, learning curve). However, controlled evaluation within specific organizational contexts remains necessary before making process changes.
...
\textbf{What We Cannot Establish}: (1) Causation - agents may not cause testing differences; developers may select agents based on task needs. (2) Quality - test presence does not indicate correctness or effectiveness. (3) Organizational behavior - no evidence teams adapt processes to agent characteristics.

\textbf{Measurement Limitations}: Keyword-based detection may include false positives/negatives. Agent labels depend on metadata/self-reports. Multiple confounders (language, project type, team culture) remain uncontrolled. Dataset aggregation may mask domain-specific patterns.

\textbf{Statistical Caveats}: Large sample size makes tiny effects significant; we report effect sizes for practical significance. No multiple comparison corrections applied. Association strength (V=0.646) does not establish causation.
